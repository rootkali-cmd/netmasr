...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rateKey = `official-vote:${ipHash}`;
  ...
  try {
    ...
    const poll = await prisma.officialPoll.findUnique({ where: { id: pollId } });
    if (!poll) {
      return NextResponse.json({ error: "الاستفتاء غير موجود" }, { status: 404 });
    }
    if (!poll.isActive) {
      return NextResponse.json({ error: "الاستفتاء غير نشط" }, { status: 400 });
    }

    const option = await prisma.officialPollOption.findFirst({
    ...
    const existingVote = await prisma.officialPollVote.findUnique({
    ...
    await prisma.$transaction([
      prisma.officialPollVote.create({
        data: { pollId, optionId, ipHash, voterId },
      }),
      prisma.officialPollOption.update({
        where: { id: optionId },
        data: { votes: { increment: 1 } },
      }),
      prisma.officialPoll.update({
        where: { id: pollId },
